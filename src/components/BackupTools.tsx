...
if (!Array.isArray(parsed.pets) || !Array.isArray(parsed.visits)) throw new Error("Formato inválido");
...
<button
className="btn"
onClick={() => downloadJson(`vet-visits-backup-${new Date().toISOString().slice(0, 10)}.json`, state)}
>
Exportar JSON
</button>
...
<span className="muted">Recomendado: exporta 1 vez al mes o antes de cambiar de PC.</span>
</div>
</section>
);
}