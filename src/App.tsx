...
const totalAll = useMemo(() => state.visits.reduce((acc, v) => acc + (v.costCLP ?? 0), 0), [state.visits]);
...
const petName = state.pets.find((p) => p.id === petId)?.name ?? "esta perrita";
const ok = confirm(`¿Eliminar ${petName}? También se eliminarán sus visitas asociadas.`);
if (!ok) return;
...
setState((s) => ({ ...s, visits: s.visits.filter((v) => v.id !== visitId) }));
}


function importState(next: AppState) {
const ok = confirm("Esto reemplazará tus datos actuales por los del backup. ¿Continuar?");
if (!ok) return;
setState(next);
setFilterPetId("");
setSearch("");
...
function resetAll() {
const ok = confirm("Esto borrará TODO (perritas y visitas) en este navegador. ¿Seguro?");
if (!ok) return;
setState(emptyState);
setFilterPetId("");
setSearch("");
...
<p className="muted">Registro simple de visitas al veterinario para tus perritas (offline).</p>
</div>
...
<span className="muted">{formatCLP(totalsByPet.get(p.id) ?? 0)}</span>
</div>
))}
</div>
</section>
...
<VisitForm pets={state.pets} defaultPetId={filterPetId || state.pets[0]?.id} onAdd={addVisit} />
...
<p className="muted small">Esto solo afecta este navegador (LocalStorage).</p>
</section>
</div>
...
Hecho para registrar visitas vet. Persistencia: LocalStorage. Exporta backups si lo necesitas.
</footer>
</div>
);
}